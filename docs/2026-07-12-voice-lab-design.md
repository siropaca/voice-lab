...
- TTS Lab / STT Lab / 履歴 の3画面を持つ Web GUI
- モデル・パラメータの GUI 切替。**同一プロバイダーの複数モデルを同時に並べて比較できる**
- 複数モデルへの並列リクエストと結果の並列表示
- レイテンシ計測
- 実行履歴のローカル保存（メトリクス + 音声）
- Cloud Run（東京リージョン）へのデプロイ
...
├── client/                  # Vite + React
│   └── src/
│       ├── pages/           # tts-lab / stt-lab / history
│       ├── components/      # モデル選択、結果カード、メトリクス表示など
│       └── lib/             # マイク取得（AudioWorklet）、ストリーミング再生、WSクライアント
├── server/                  # Hono + ws
│   └── src/
│       ├── adapters/
...
│       ├── routes/          # /api/models, /api/tts, /ws/stt, /api/history
│       └── history.ts       # 実行履歴の保存・読み出し
├── data/                    # 実行履歴（runs.jsonl + audio/*.wav 等）※ .gitignore
└── docs/
```
...
- モデル選択（複数チェック。同一プロバイダーの複数モデルも可）と、モデルごとのパラメータ調整（声、速度など。レジストリのスキーマから動的生成）
- 「合成」ボタンで選択モデルへ並列リクエスト
- 結果はモデルごとのカードを横に並べ、各カードに:
  - 再生ボタン（ストリーミング再生。全カード順次再生ボタンも用意）
...
- モデル選択（複数チェック。同一プロバイダーの複数モデルも可）
- 「開始」でマイク取得（AudioWorklet で 16kHz 16bit PCM mono に変換）→ WebSocket でサーバーへ送信 → サーバーが選択モデル全てへ同時配信（ファンアウト）
- モデルごとの列を並べ、逐次表示:
  - partial（途中結果）はグレー、final（確定）は黒で追記
  - 各 partial/final の受信タイムスタンプを記録
- 「停止」時に:
  - 発話終了 → 最終確定までの遅延を表示
  - 最終テキストを全モデル分並べて比較（正解文の目視比較用に、話した内容のメモ欄を任意入力）
- 録音した音声も履歴に保存し、後から聞き直せる

### 5.3 履歴

- 過去の実行（TTS 合成 / STT セッション）の一覧と詳細表示
- 音声の再生、メトリクスの確認ができる
- 保存形式は `data/runs.jsonl` + `data/audio/` のフラットファイル。DB は使わない
...
ネットワーク条件で値が揺れるため、履歴に残して複数回の実行を見比べられるようにする（統計処理は v1 ではしない）。
...
開発中はローカル実行（`pnpm dev`）が基本。共有・スマホ実機での確認用に **Cloud Run（asia-northeast1 / 東京）** へデプロイする。
...
4. 履歴
5. Cloud Run デプロイ（GCS マウント + Basic 認証）

詳細は実装計画（writing-plans）で分解する。
