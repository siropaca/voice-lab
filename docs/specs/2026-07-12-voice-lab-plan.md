...
**Goal:** 日本語 TTS/STT クラウド API を品質・レイテンシ・ストリーミング対応で並列比較する Web GUI（TTS Lab / STT Lab / 履歴）を作る。ローカル実行のみ（デプロイはフェーズ2 = Task 18〜19 で、第一フェーズでは実行しない）。
...
### Task 18: 本番配信モードと Basic 認証（TDD）【フェーズ2 — 第一フェーズでは実行しない】
...
### Task 19: Dockerfile と Cloud Run デプロイ【フェーズ2 — 第一フェーズでは実行しない】
...
| 4. 履歴 | Task 17 |
| （フェーズ2）Cloud Run デプロイ | Task 18〜19 — **第一フェーズでは実行しない**。第一フェーズの完了は Task 17 まで |
