...
- **Google STT のリージョンはモデル固有**。Chirp 3 は `us`/`eu` マルチリージョンのみで `global` 不可（実測確認済み）。`latest_long` は `global` で提供。registry の `location` でモデルごとに指定し、無指定は `GOOGLE_SPEECH_LOCATION`（既定 `us`）に従う。別モデル/リージョンを試すのも registry 1 エントリで足せる。
