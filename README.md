...
- **Google STT のリージョンはモデル固有**。Chirp 3 は `us`/`eu` マルチリージョンのみで `global` 不可（実測確認済み）。`latest_long` は `global`、Chirp 2 は `us-central1`。registry の `location` でモデルごとに指定する（無指定は `us`）。別モデル/リージョンを試すのも registry 1 エントリで足せる。
